...
const upload = multer({
    ...
    fileFilter: function (req, file, callback) {
        ...
        if(ext !== ".png" && ext !== ".jpg" && ext !== ".gif" && ext !== ".jpeg") {
            const response = {"statusCode": 406, "statusMessage": "NOT ACCEPTABLE", "responseMessage": "Please select images."};
...
function uploadImages(detectionly) {
    ...
    detectionly.post("/api/v1/repository/upload/", upload.array("images", 2), function (req, res) {
        // In case no file is selected, return NOT ACCEPTABLE response.
        const files = req.files;
        if (!files || files.length === 0) {
            return res.status(406).send({"statusMessage": "NOT ACCEPTABLE", "responseMessage": "Please select images."});
        }
        ...
        if (isAuthorized === false) {
            ...
            fs.readdir(imageUploadTemporaryDirectory, (error, files) => {
                ...
                files.forEach(file => {
                    fs.unlink(path.join(imageUploadTemporaryDirectory, file), (error) => {
                        if (error) {
                            console.error(error);
                        }
                    });
