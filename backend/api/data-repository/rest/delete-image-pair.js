...
function deleteImagePair(detectionly) {
    ...
    detectionly.delete("/api/v1/repository/delete/", function (req, res) {
        ...
        // Get name of image pair folder from req.query and construct absolute path to it.
        ...
        if (nameOfImagePairFolder === null || nameOfImagePairFolder === "") {
            return res.status(400).send({"statusMessage": "BAD REQUEST", "responseMessage": "Image Pair Folder Name Cannot Be Empty"});
        }

        // Check if image pair folder exists on filesystem.
        fs.access(imagePairToDelete, fs.constants.F_OK, (error) => {
            // If image does not exist, return NOT FOUND response.
            if (error) {
            ...
            fs.rmdir(imagePairToDelete, {recursive: true}, (error) => {
                ...
                if (error) {
                    return res.status(500).send({"statusMessage": "INTERNAL SERVER ERROR", "responseMessage": "Cannot Delete Image Pair"});
                }
                // Return OK response.
                return res.status(200).send({"statusMessage": "OK", "responseMessage": "Image Pair Deletion Successful"});
            });
        });
    });
}
