...
function locateImages(detectionly) {
    ...
    detectionly.get("/api/v1/repository/locate/all", function (req, res) {
        ...
        if (isAuthorized === false) {
            return res.status(401).send({"statusMessage": "UNAUTHORIZED", "responseMessage": "Invalid / Expired Token"});
        }
        ...
        if (!fs.existsSync(userDataDirectory)) {
            return res.status(404).send({"statusMessage": "NOT FOUND", "responseMessage": "User Data Directory Not Found", "userUploadedImagesPairFolderPaths": null, "userUploadedImagesPairPaths": null});
        }

        // Read user data directory,
        fs.readdir(userDataDirectory, null, (error, userUploadedImagesPairFolders) => {
            // If error return INTERNAL SERVER ERROR response.
            if (error) {
                console.error(error);
                return res.status(500).send({"statusMessage": "INTERNAL SERVER ERROR", "responseMessage": "An Error Occurred While Reading User Data Directory", "userUploadedImagesPairFolderPaths": null, "userUploadedImagesPairPaths": null});
            }

            // If no images are found in directory, return NOT FOUND response.
            if (userUploadedImagesPairFolders.length === 0) {
                return res.status(404).send({"statusMessage": "NOT FOUND", "responseMessage": "No Image Found", "userUploadedImagesPairFolderPaths": null, "userUploadedImagesPairPaths": null});
            }
            ...
            return res.status(200).send({"statusMessage": "OK", "responseMessage": "Images Retrieved", "userUploadedImagesPairFolders": userUploadedImagesPairFolders, "userUploadedImagesPairPaths": userUploadedImagePairPaths});
        });
    });
}
