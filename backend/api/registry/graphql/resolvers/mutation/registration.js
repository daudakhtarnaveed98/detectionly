...
async function registerUser(userRegistrationData) {
  ...
  if (!userRecordExistsInDatabase) {
    ...
    try {
      isAllUserRegistrationDataProvided = await utils.checkIfAllUserRegistrationDataIsProvided(
    ...
    if (isAllUserRegistrationDataProvided) {
      ...
      try {
        userRegistrationData.password = await hash(
  ...
  // Return CONFLICT response if userRecordExistsInDatabase is true.
    ...
    return {
      statusCode: commons.statusCodes.CONFLICT,
